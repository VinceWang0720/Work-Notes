		...
		return search.create({
			type: "item",
				filters: [
					...
					"AND",
					["internalid","anyof","1779","1780","1554","1559"]
					//["internalid","anyof","1559"]
					//["internalid","noneof","565","575","578","613","631","947","953","954","955","956","987","988",
					 //"990","994","995","1003","1006","1009","1010","1018","1019","1039","1057","1058","1059","1068",
					 //"1104","1110","1121","1148","1152","1154","1175","1183","1203","1214","1223","1224","1225","1228",
					 //"1235","1237","1259","1356","1418","1419","1420","1426","1454","1456","1498","1500","1513","1543",
					 //"1546","1557","1562","1568","1585","1586","1587","1591","1592","1597","1609","1619","1648","1649",
					 //"1650","1652","1653","1654","1655","1656","1657","1658","1661","1678","1712","1715","1716","1766",
					 //"1802","1824","1834","1841","1844","1852","1869","1873","1879","1884","1890","1901","1930","1939","1948"]
				],
	   		columns:
	   		[
			...
			if( searchObj.runPaged().count > 0 ){
				...
				location_name.forEach(function (result, location_index) {
					...
					if( location_onHand[location_index] != 0 )
					{
						...
						log.debug("map dayArray",dayArray);
						log.debug("map writePeriod",writePeriod);

						//紀錄againg和Inv_QTY
						var data = {};
						for (var i = 0; i < PERIOD_ARR.length; i++) {
							var key = "_" + PERIOD_ARR[i].from + "_" + PERIOD_ARR[i].to;
							data[key] = {
							  'againg': dayArray[i],
							  'inv_qty': writePeriod[i]
							};
						  }	
			...
			if(context.values.length == 1){
				...
				for(var i = 0 ; i < context.values.length; i++){
					...
					if(!obj.PeriodObject){
							  ...
							  if(PeriodObject[per].length > 0 && Number(Period["onHand_temp"]) > 0){
								  //log.debug("尋找其他倉"+per,PeriodObject[per]);
								   for(var l = 0 ; l < PeriodObject[per].length; l++){
									   ...
									   PeriodDate1 =PeriodObject[per][l]["trandate"];
									   if(Number(Period["onHand_temp"]) >= Number(PeriodObject[per][l]["sum"])){
										   PERIOD_ARR.forEach(function (result, index) {
											   ...
											   if(PeriodDate<=newdate && PeriodDate>= newdate2){
													...
													var keyToCheck = "_"+from+"_"+to;
													if (keyToCheck in Period.Data) {
														Period.Data[keyToCheck].inv_qty = Period["_"+from+"_"+to];
														Period.Data[keyToCheck].againg = PeriodDate1;
													}
					   ...
					   //log.debug("dreduce dayArray",dayArray);
					...
					// var i = Object.keys(Period.Data).length + 1; // 開始的索引數
					// while (newWritePeriod.length > 0) {
					// 	var key = "data" + i;
					// 	if (!Period.Data.hasOwnProperty(key)) {
					// 		var value = newWritePeriod.shift(); // 取出第一個元素
					// 		Period.Data[key] = {
					// 			'againg': Period.Data.data1.againg,
					// 			'inv_qty': value
					// 		};
					// 	}
					// 	i++;
		...
		try{
			...
			for (var key in detail.Data) {
				if (detail.Data.hasOwnProperty(key)) {
				  if (detail.Data[key].inv_qty === 0) {
					delete detail.Data[key];
				  }
				}
			}

			log.debug("detail.Data Done",detail.Data);
			...
			for(var key in detail.Data){
				...
				if (detail.Data.hasOwnProperty(key)) {
					...
					xmlString += '<Cell ss:StyleID="number1"><Data ss:Type="Number">' + NVL(detail.ttlQTY) + '</Data></Cell>' +
